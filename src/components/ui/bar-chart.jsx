...
function TooltipDot({
  ...
  return (
    <motion.circle cx={animatedX} cy={animatedY} fill={color} r={size} stroke={strokeColor} strokeWidth={strokeWidth} />
  )
}
...
function TooltipContent({ title, rows, children }) {
  ...
  const isWaitingForSettlement =
    committedChildrenStateRef.current !== null && committedChildrenStateRef.current !== hasChildren
  ...
  return (
    ...
    >
      <div className="px-3 py-2.5" ref={measureRef}>
        {title && <div className="mb-2 font-medium text-popover-foreground text-xs">{title}</div>}
...
export function BarXAxis({ tickerHalfWidth = 50, showAllLabels = false, maxLabels = 12 }) {
...
export function BarYAxis({ showAllLabels = true, maxLabels = 20 }) {
...
export function LegendItemComponent({ className = '', children }) {
...
export function LegendMarker({ className = '' }) {
  ...
  if (!item) return null
  return <span className={cn('h-3 w-3 shrink-0 rounded-sm', className)} style={{ backgroundColor: item.color }} />
}
...
const DEFAULT_MARGIN = { top: 24, right: 16, bottom: 36, left: 56 }
    ...
    <BarChartProvider value={contextValue}>
      <svg aria-hidden="true" height={height} width={width}>
        ...
        <g
          ...
          style={{ cursor: isLoaded ? 'crosshair' : 'default', touchAction: 'none' }}
...
export function BarChart({
  ...
  return (
    <div
      ...
      ref={containerRef}
      style={{ aspectRatio: aspectRatio || undefined, touchAction: 'none' }}
    >
